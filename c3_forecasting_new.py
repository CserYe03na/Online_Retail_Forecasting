...
"""C3 forecasting comparison script built from scratch.

This module compares several leak-safe daily forecasting methods for cluster C3:
...
The implementation uses only training history for feature construction.
For the count model, all test-period features are built recursively from prior predictions.
"""
...
PREDICTION_OUTPUT_DEFAULT = "forecasting/c3_prediction_new.parquet"
METRICS_OUTPUT_DEFAULT = "forecasting/c3_metrics_new.csv"
...
def _build_train_features(train_panel: pd.DataFrame) -> pd.DataFrame:
    ...
    df["days_since_last_sale_clip84"] = np.minimum(df["days_since_last_sale"], 84)
...
def _forecast_tsb(series: np.ndarray, horizon: int, alpha: float = 0.15, beta: float = 0.05) -> np.ndarray:
...
def _forecast_imapa(series: np.ndarray, horizon: int, aggregation_levels: Sequence[int] = (7, 14, 28)) -> np.ndarray:
    forecasts = [_forecast_adida(series, horizon, k=k) for k in aggregation_levels]
    return np.mean(np.vstack(forecasts), axis=0) if forecasts else np.zeros(horizon, dtype=float)
...
def _clip_count_target(y: np.ndarray, q: float = 0.98, max_cap: int = 120) -> np.ndarray:
...
def _is_binary_or_cyclic_feature(col: str) -> bool:
    return (
        ...
        or col in {"is_weekend", "dow", "month", "weekofyear"}
    )
...
def _prepare_design_matrices(
    ...
    x_raw, feature_center, feature_scale = _stabilize_feature_frame(
        df, COUNT_FEATURE_COLS, center=feature_center, scale=feature_scale
    )
...
def _fit_zinb(train_feat: pd.DataFrame, maxiter: int = 250) -> Dict[str, Any]:
    y = _clip_count_target(train_feat["y"].to_numpy(dtype=float))
    x, x_infl, design_info = _prepare_design_matrices(train_feat)
    ...
    try:
        ...
        return {"result": result, "status": "ok", **design_info}
    except Exception as exc:  # pragma: no cover - defensive runtime fallback
        warnings.warn(f"ZINB fit failed; using zero forecast fallback. Error: {exc}")
        return {"result": None, "status": f"failed: {exc}", **design_info}
...
def _predict_zinb(fit_obj: Mapping[str, Any], feat_df: pd.DataFrame) -> Tuple[np.ndarray, np.ndarray]:
    if fit_obj.get("result") is None:
        return np.zeros(len(feat_df), dtype=float), np.zeros(len(feat_df), dtype=float)
    ...
    return np.clip(mean_pred, 0.0, None), np.clip(p_nonzero, 0.0, 1.0)
...
def _recursive_zinb_forecast(
    ...
    for current_date in test_dates:
        ...
        mean_pred, p_nonzero = _predict_zinb(fit_obj, feat_df)
...
def run_c3_forecasting_new(
    ...
    zinb_fit = _fit_zinb(train_features)
    ...
    pred_df["p_sale"] = pred_df["p_nonzero_zinb"].fillna(0.0)
...
def _build_arg_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Leak-safe C3 model comparison pipeline")
    parser.add_argument("--train-path", default=TRAIN_DEFAULT)
    parser.add_argument("--test-path", default=TEST_DEFAULT)
    parser.add_argument("--cluster-id", type=int, default=3)
    parser.add_argument("--zinb-threshold", type=float, default=0.60)
