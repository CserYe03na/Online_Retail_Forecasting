...
- `Future mode`: forecasts beyond the test period using train + test history
- `Evaluation mode`: reproduces test-period forecasts with actual values when available
...
- cluster assignment
- routed production model
- forecast table
- forecast chart

## Cluster Routing In Production

The agent uses fixed routing by cluster:
...
```bash
PYTHONPATH=. python -m pytest agent/tests
```

### Execute a notebook in the conda environment

```bash
PYTHONNOUSERSITE=1 python -s -m nbconvert \
  --to notebook \
  --execute \
  --inplace \
  --ExecutePreprocessor.kernel_name=forecasting-retail \
  cluster_analysis.ipynb
```
